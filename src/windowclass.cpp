 ...
 * windowclass.cpp
 *
 * Last modified: <2014/01/30 19:55:28 +0900> By Zumida
 */
...
namespace swo {
	inline namespace window {
		...
		WindowClass::ExistsClassException::ExistsClassException(
			const String& className)
			: Exception(className) {}

		WindowClass::RegisterClassException::RegisterClassException(
			const String& className)
			: Exception(className) {}

		WindowClass::WindowClass()
			: icon(nullptr), smallIcon(nullptr), cursor(nullptr),
		background(nullptr), wndProc(nullptr) {}
		...
		void WindowClass::add(class WindowClass& wndClass) {
			// 同一クラス名の登録チェック
			if (find(wndClass.className) != nullptr) {
				throw ExistsClassException(wndClass.className);
			}
			...
			if (atom == 0) {
				throw RegisterClassException(wndClass.className);
			}
